	...
	<form name="frm" id="theform" method="post">
		<table style="float: right;">
			<tr>
				...
				<td>사업자 이름 <input type="text" name="key" id="key"
					value="${key}"> <input class="btn" type="button" value="검색"
					onclick="go_search()">
				</td>
			</tr>
		</table>
		...
		<table id="orderList">
			...
			<c:forEach items="${hostList}" var="hostVO">
				<tr>
					<!-- check boxes for status and delete - should switch with the button on top -->
					<td name="statuscheck" style="display: inline"><c:choose>
					...
					<td name="deletecheck" style="display: none"><input
						type="checkbox" name="delete" value="${hostVO.hemail}"></td>
					<td>${hostVO.name}</td>
					<td>${hostVO.hemail}</td>
					<td>${hostVO.phone}</td>
				</tr>
		...
		<input type="button" id="button3" class="btn" style="width: 200px"
			value="HOST 승인" onClick="approve_host()"> <input
			type="button" id="button4" class="btn"
			style="width: 200px; display: none" value="HOST 삭제"
			onClick="delete_host()">
	</form>
