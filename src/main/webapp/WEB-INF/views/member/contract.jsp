...
제 1 조 (목적) 
이 이용약관(이하 '약관')은 주식회사 Kozynest(이하 회사라 합니다)와 이용 고객(이하 '회원')간에 회사가 제공하는 jjang0u.com서비스의 가입조건 및 이용에 관한 다음의 제반 사항과 기타 기본적인 사항을 구체적으로 규정함을 목적으로 합니다.

제 2 조 (이용약관의 효력 및 변경) 
(1) 이 약관은 Kozynest 웹사이트(www.wownoage.com 이하 'Kozynest 웹')에서 온라인으로 공시함으로써 효력을 발생하며, 합리적인 사유가 발생할 경우 관련법령에 위배되지 않는 범위 안에서 개정될 수 있습니다. 개정된 약관은 온라인에서 공지함으로써 효력을 발휘하며, 이용자의 권리 또는 의무 등 중요한 규정의 개정은 사전에 공지합니다.
...
제 4 조 (용어의 정의) 
(1) "이용자" 또는”회원”이란 "Kozynest"에 접속하여 본 약관에 따라 "Kozynest"에 가입하여, 이용 고객의 식별과 이용고객의 서비스 이용을 위하여 이용고객 본인이 직접 선정하고 회사가 부여하는 문자와 숫자 조합인 ‘이용자번호(ID)’와 그에 따라 일치된 이용고객임을 확인하기 위하여 이용고객 본인이 선정한 ‘비밀번호’를 이용하여 "Kozynest"이 제공하는 서비스를 받는 자를 말합니다.
(2) 이 약관에서 사용하는 용어의 정의는 제1항에서 정하는 것을 제외하고는 관계법령 및 서비스별 안내에서 정하는 바에 의합니다. 

제 2 장 이용계약 체결

제 5 조 (이용 계약의 성립)
(1) 이용계약은 이용고객의 본 이용약관 내용에 대한 동의와 이용신청에 대하여 회사의 이용승낙으로 성립하여 이용약관에 대한 동의는 이용신청 당시 해당 Kozynest 웹의 '동의함' 버튼을 누름으로써 의사표시를 합니다.
...
제 7 조 (이용자의 개인 정보 보호) 
회사는 관련법령이 정하는 바에 따라서 이용자 등록정보를 포함한 이용자의 개인정보를 보호하기 위하여 노력합니다. 이용자의 개인정보보호에 관해서는 관련법령 및 "Kozynest"이 정하는 "개인정보보호정책"에 정한 바에 의합니다.

제 8 조 (이용 신청의 승낙과 제한) 
(1) 이용자가 되고자 하는 자는 회사가 정한 가입 양식에 따라 회원정보를 기입하고 "등록하기" 단추를 누르는 방법으로 회원 가입을 신청합니다.
(2) 회사는 아래사항에 해당하는 경우에 대해서 승낙하지 아니 합니다.
ㄱ. 가입신청자가 본 약관 의거하여 이전에 회원자격을 상실한 적이 있는 경우. 상실 후 1개월 경과 하지 않은  자로서 "Kozynest"의 회원재가입 승낙을 얻으려 하는 경우
ㄴ. 등록 내용에 허위, 기재누락, 오기가 있는 경우 
ㄷ. 사회의 안녕과 질서, 미풍양속을 저해할 목적으로 신청한 경우
ㄹ. 부정한 용도로 본 서비스를 이용하고자 하는 경우 
ㅁ. Kozynest의 정책과 위배 되어지는 행위를 반복적으로 자행하여 회원 자격 상실을 한 적이 있는 경우
...
(3) ‘Kozynest’의 이용자ID는 이용자 본인의 동의하에 ‘Kozynest’ 가 운영하는 자사 사이트의 회원ID와 연결될 수 있습니다.
...
제 12 조 (서비스 이용 시간) 
(1) 서비스 이용은 회사의 업무상 또는 기술상 특별한 지장이 없는 한 연중무휴, 1일 24시간 운영을 원칙으로 합니다. 단, 회사는 시스템 정기점검, 증설 및 교체를 위해 회사가 정한 날이나 시간에 서비스를 일시 중단할 수 있으며, 예정되어 있는 작업으로 인한 서비스 일시 중단은 Kozynest 웹을 통해 사전에 공지합니다.
...
제 18 조 (계약 변경 및 해지)
회원이 이용계약을 해지하고자 하는 때에는 회원 본인이 Kozynest 웹 내의 [도움말] 메뉴를 이용해 가입해지를 해야 합니다. 
...
(14) 회사는 회원이 Kozynest을 이용함으로써 회원 본인 또는 제3자에게 직/간접적으로 영향을 미치게 되는 모든 피해와 법적 문제에 대하여 책임을 지지 않습니다.
